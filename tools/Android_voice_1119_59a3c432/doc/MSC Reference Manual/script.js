...
function gShowSchApply(){
	if( gIsIflytek() ){
		document.write( "自 4.5.1116.XXXX 版本开始，语义启用 3.0 版本，加入支持连续的上下文交互等优化，建议所有应用在新的版本中使用新的 3.0 的语义。申请SDK时，默认未开通语义功能，如需使用请在<a href="+gIflySchWebsiteLink+" target=\"_blank\">"+gIflySchWebsiteName+"</a>上进行业务配置" );
	}else{
...
function gShowSetupNlpScene(){
	if( gIsIflytek() ){
		document.write( "语义情景请通过<a href="+gIflySchWebsiteLink+" target=\"_blank\">"+gIflySchWebsiteName+"</a>上进行配置。" );
	}else{
		document.write( "此参数在灵犀云上暂不支持。" );
	}
}

function gShowDefaultNlpVersion(){
	if( gIsIflytek() ){
		document.write( "\"3.0\"" );
	}else{
		document.write( "\"2.0\"" );
...
function gShowUnSupportInLxy(){
	if(  !gIsIflytek() ){
		document.write( "<p>此功能在灵犀云上暂不支持。</p>" );
	}
}
/*********************************	自定义函数结束	***************************************/